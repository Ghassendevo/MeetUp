...
$_pic = $g['pic'];
$s = mysqli_query($con,"SELECT * FROM followers WHERE follow_to='$id' ") or die(mysqli_error($con));
if(mysqli_num_rows($s)==0){
	...
	<div style="background-color: #f0f2f5;width: 100%;border-radius: 10px;height: 37px;">
		<div style="display: flex;justify-content: space-between;margin-left: 10px;">
			<i class="fas fa-search" style="color: #8f959c;margin-top: 7px;"></i><input type="text" id="fo_search" class="fo_search" onkeyup="search()" placeholder="Search followers..." name="" style=""><i class="fas fa-times te" style="color: black;margin-top: 7px;margin-right: 5px;cursor: pointer;visibility: hidden;"></i>
		</div>
		<div class="data_search" style=" width: 100%;margin: auto;margin-top: 15px;height: 55vh;overflow: hidden;overflow-y: auto;">
				...
				while ($f = mysqli_fetch_array($s)) {
					$follow_by = $f['follow_by'];
					$se = mysqli_query($con,"SELECT * FROM users WHERE id='$follow_by'") or die(mysqli_error($con));
